...
export class CardList extends React.Component {
  ...
  //display cards
  userSelectLevel = () => {
    return this.state.showCards ? (null) : (<div className='card-list'>
      {this.state.randomNumbers.map(number =>
        <Card key={number} number={number} updateSelectedInput={this.updateSelectedInput} />)}
    </div>)
  }
  ...
  render() {
    return (<main>
      <div>
        ...
        {(this.state.isWon && this.state.showMessage) ? (<Message message='You won Game!' click={this.backToSelectLevel} />) : null}
        {(!this.state.isWon && this.state.showMessage) ? (<Message message='You lose Game!' click={this.backToSelectLevel} />) : null}
        <div className='card-buttons'>
          <button className='play-button' id='back'
            onClick={this.backToSelectLevel}>Back</button>
          <Link to='/' children={GameInstruction}>
            <button className='play-button' id='back'>Help</button>
          </Link>
        </div>
      </div>
